        ...
        *,
        ...
        if not (x.is_nested ^ residual.is_nested):
            if not x.is_nested:
                ...
                save_tensors(torch.nested.nested_tensor(list(x.unbind(0))), "6")
        ...
        if not (x.is_nested ^ residual.is_nested):
            if not x.is_nested:
                ...
                x = torch.nested.as_nested_tensor([x_ + residual_ for (x_, residual_) in zip(x.unbind(0), residual.unbind(0))], layout=torch.jagged)
                save_tensors(torch.nested.nested_tensor(list(x.unbind(0))), "5")
        else:
            raise ValueError("Either of the two, X or residual, is not nested... the other is")
...
class LLaMA(nn.Module):
    ...
    def _helper(
        ...
        if not x_in.is_nested: 
            qlen = x_in.size(1)
            klen = x_in.size(1)
            save_tensors(torch.rand(2, 3), "3_1")
            save_tensors(torch.rand(2, 3), "3_2")
        else:

    ...
    def adjusted_qk(
        ...
        if not (k.is_nested ^ q.is_nested):
            if not k.is_nested:
                ...
                save_tensors(q, "9_e")
                save_tensors(q_, "9_f")
                save_tensors(freqs, "9_g")
                temp = freqs[:, -q.size(1) :, None, :, :, :]
                save_tensors(temp, "9_a")
                temp = temp.mul(q_.unsqueeze(-2))
                save_tensors(temp, "9_b")
                temp = temp.sum(5)
                save_tensors(temp, "9_c")
                temp = temp.flatten(3)
                save_tensors(temp, "9_d")
                q_out = temp.type_as(q)
                ...
                save_tensors(q_out, "9_1")
                save_tensors(k_out, "9_2")
                ...
                for i, (q_ele, q_ele_) in enumerate(zip(q.unbind(0), q_.unbind(0))):
                    templist_.append(freqs[i, -q_ele.size(0) :, None, :, :, :])
                temp = torch.nested.nested_tensor(templist_)
                save_tensors(temp, "9_a")

                templist_ = []
                for i, (temp_, q_ele, q_ele_) in enumerate(zip(temp.unbind(0), q.unbind(0), q_.unbind(0))):
                    templist_.append(temp_.mul(q_ele_.unsqueeze(-2)))
                temp = torch.nested.nested_tensor(templist_)
                save_tensors(temp, "9_b")

                templist_ = []
                for i, (temp_, q_ele, q_ele_) in enumerate(zip(temp.unbind(0), q.unbind(0), q_.unbind(0))):
                    templist_.append(temp_.sum(4))
                temp = torch.nested.nested_tensor(templist_)
                save_tensors(temp, "9_c")

                templist_ = []
                for i, (temp_, q_ele, q_ele_) in enumerate(zip(temp.unbind(0), q.unbind(0), q_.unbind(0))):
                    templist_.append(temp_.flatten(2))
                temp = torch.nested.nested_tensor(templist_)
                save_tensors(temp, "9_d")

                templist_ = []
                for i, (temp_, q_ele, q_ele_) in enumerate(zip(temp.unbind(0), q.unbind(0), q_.unbind(0))):
                    templist_.append(temp_.type_as(q_ele))
                q_out = torch.nested.nested_tensor(templist_)

                q_out = torch.stack([
                    (freqs[b:b+1, -q[b].size(0):, None, :, :, :]
                    .mul(q_[b].unsqueeze(-2))
                    .sum(4)
                    .flatten(2)
                    ).type_as(q[b])
        ...
        if not (k.is_nested ^ q.is_nested):
            if not k.is_nested:
                temp = q_out.view_as(q), k_out.view_as(k)
                save_tensors(temp[0], "10_1")
                save_tensors(temp[1], "10_2")
                return temp
            else:
                temp = torch.nested.nested_tensor([q_out_.view_as(q_) for q_, q_out_ in zip(q.unbind(0), q_out.unbind(0))]), torch.nested.nested_tensor([k_out_.view_as(k_) for k_, k_out_ in zip(k.unbind(0), k_out.unbind(0))])
                save_tensors(temp[0], "10_1")
                save_tensors(temp[1], "10_2")
                return temp

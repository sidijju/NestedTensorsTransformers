    ...
    def forward(
        ...
        if is_self or past_key_value_state is None:
            ...
            if not q.is_nested:
                ...
                keys = k_out.view(batch_size, q_len, self.kvheads, self.emb_kq_per_head)
                values = v_out.view(batch_size, q_len, self.kvheads, self.emb_v_per_head)
                save_tensors(queries, "5_1")
                save_tensors(keys, "5_2")
                save_tensors(values, "5_3")
            else:
        ...
        if mask is not None:
            ...
            if not mask.is_nested:
                while len(mask.size()) != 4:  # expects bs (x nheads) x q_len x kv_len
                    mask = mask.unsqueeze(1)
                save_tensors(mask, "11")
                
            else:
                while len(mask.unbind(0)[0].size()) + 1 != 4:  # expects bs (x nheads) x q_len x kv_len
                    mask = torch.nested.nested_tensor([ele.unsqueeze(1) for ele in mask.unbind(0)])
                save_tensors(mask, "11")
